...
type Snapshot struct {
	config   *params.YogurtConfig // Consensus engine parameters to fine tune behavior
...
// the genesis block.
func newSnapshot(config *params.YogurtConfig, sigcache *sigLRU, number uint64, hash common.Hash, signers []common.Address) *Snapshot {
	snap := &Snapshot{
		config:   config,
		sigcache: sigcache,
		Number:   number,
...
func loadSnapshot(config *params.YogurtConfig, sigcache *sigLRU, db ethdb.Database, hash common.Hash) (*Snapshot, error) {
