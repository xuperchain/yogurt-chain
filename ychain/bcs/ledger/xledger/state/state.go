...
import (
	...
	"github.com/xuperchain/yogurt-chain/ychain/bcs/ledger/xledger/state/context"
	"github.com/xuperchain/yogurt-chain/ychain/bcs/ledger/xledger/state/meta"
	...
	"github.com/xuperchain/yogurt-chain/ychain/bcs/ledger/xledger/state/xmodel"
	"github.com/xuperchain/yogurt-chain/ychain/bcs/ledger/xledger/tx"
	txpkg "github.com/xuperchain/yogurt-chain/ychain/bcs/ledger/xledger/tx"
	pb "github.com/xuperchain/yogurt-chain/ychain/bcs/ledger/xledger/xldgpb"
	...
	pb2 "github.com/xuperchain/yogurt-chain/ychain/kernel/contract/bridge/pb"
	...
	"github.com/xuperchain/yogurt-chain/ychain/lib/cache"
...
const (
	...
	autoGenRandom = "AutoGen.SubmitRandomNumber"
)
...
func (t *State) GetRandomTx(blockHeight uint64, args map[string][]byte) (*pb.Transaction, error) {
	stateConfig := &contract.SandboxConfig{
		XMReader:   t.CreateXMReader(),
		UTXOReader: t.CreateUtxoReader(),
	}
	...
	t.log.Info("XRandom", "SubmitRandomNumber", blockHeight)
	sandBox, err := t.sctx.ContractMgr.NewStateSandbox(stateConfig)
	if err != nil {
		t.log.Error("PreExec new state sandbox error", "error", err)
		return nil, err
	...
	ctx, err := t.sctx.ContractMgr.NewContext(contextConfig)
	if err != nil {
		t.log.Error("SubmitRandomNumber NewContext error",
			"err", err, "contractName", req.GetContractName())
		return nil, err
	}
	...
	if ctxErr != nil {
		ctx.Release()
		t.log.Error("SubmitRandomNumber Invoke error",
			"error", ctxErr, "contractName", req.GetContractName())
		return nil, ctxErr
	}
	// 判断合约调用的返回码
	if ctxResponse.Status >= 400 {
		ctx.Release()
		t.log.Error("SubmitRandomNumber Invoke error",
			"status", ctxResponse.Status, "contractName", req.GetContractName())
		return nil, errors.New(ctxResponse.Message)
	}
	...
	autoTx.Desc = []byte(autoGenRandom)
	txID, err := txhash.MakeTransactionID(autoTx)
	autoTx.Txid = txID

	t.log.Trace("SubmitRandomNumber", "readSet", rwSet.RSet, "writeSet", rwSet.WSet)

	return autoTx, nil
}

func (t *State) QueryRandom(args map[string][]byte) ([]byte, error) {
	stateConfig := &contract.SandboxConfig{
		XMReader:   t.CreateXMReader(),
		UTXOReader: t.CreateUtxoReader(),
	}
	...
	t.log.Info("XRandom", "QueryRandomNumber arg", args)
	sandBox, err := t.sctx.ContractMgr.NewStateSandbox(stateConfig)
	if err != nil {
		t.log.Error("PreExec new state sandbox error", "error", err)
		return nil, err
	...
	ctx, err := t.sctx.ContractMgr.NewContext(contextConfig)
	if err != nil {
		t.log.Error("QueryRandomNumber NewContext error",
			"err", err, "contractName", req.GetContractName())
		return nil, err
	}
	...
	if ctxErr != nil {
		ctx.Release()
		t.log.Error("QueryRandomNumber Invoke error",
			"error", ctxErr, "contractName", req.GetContractName())
		return nil, ctxErr
	}
	// 判断合约调用的返回码
