...
func (t *RpcServMG) runRpcServ() error {
	...
	// Xrandom service
	xrandomService, err := newRandomService(t.scfg, t.engine)
	if err != nil {
		t.log.Error("failed to register xrandom service", "err", err)
		return fmt.Errorf("failed to register xrandom service")
	}
	pb.RegisterRandomServer(t.servHD, xrandomService)
