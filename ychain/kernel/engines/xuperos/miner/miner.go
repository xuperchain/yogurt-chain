...
func (m *Miner) step() error {
	...
	db := m.ctx.State.CreateXMReader()
	pre, _ := xrandom.GetRandomFromDB(db, uint64(ledgerTipHeight-1))
	now, _ := xrandom.GetRandomFromDB(db, uint64(ledgerTipHeight))
	ctx.GetLog().Debug("xrandom height and random",
		"height", ledgerTipHeight,
		"previous", pre,
		"now", now)
...
func (m *Miner) packBlock(ctx xctx.XContext, height int64,
	...
	// 1.1 生成 xrandom 交易
	randomTx, err := xrandom.GetTx(ctx, height)
	if err != nil {
		// TODO
		ctx.GetLog().Debug("xrandom.GetTx() failed", "error", err)
	}
	ctx.GetLog().Debug("xrandom.GetTx() succ",
		"tx", randomTx)
	...
	// 再auto tx
	if randomTx != nil && len(randomTx.TxOutputsExt) > 0 {
		ctx.GetLog().Debug("pack random tx to tx list")
		txList = append(txList, randomTx)
	}
	if len(autoTx.TxOutputsExt) > 0 {
